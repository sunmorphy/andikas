...
async function handleAutoTranslate() {
  ...
  const hasEnCompany = form.value.companyName.en?.trim()
  const hasEnLocation = form.value.location.en?.trim()
  const hasEnDesc = form.value.description.en?.trim()
  
  if (!hasEnCompany && !hasEnLocation && !hasEnDesc) {
  ...
  try {
    ...
    if (hasEnCompany) {
      promises.push(translateText(form.value.companyName.en))
