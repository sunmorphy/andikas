...
export default function ProjectsClient({
  ...
  const backToHomeHref = lang === "en" ? "/" : `/?lang=${lang}`;
        ...
        >
          <div>
            {currentPage > 1 ? (
              ...
              >
                ← {(dict.projects?.previous || "previous").toLowerCase()}
              </button>
            ) : (
          ...
          <div className="flex items-center gap-2 font-medium text-ink/70">
            <span>
              {(dict.projects?.page || "page").toLowerCase()} {currentPage}{" "}
              {(dict.projects?.of || "of").toLowerCase()} {initialMeta.totalPages}
            </span>
          </div>
          ...
          <div>
            {currentPage < initialMeta.totalPages ? (
              <button
                ...
                className="hover:text-brand-900 underline underline-offset-4 decoration-1 transition-colors cursor-pointer"
              >
                {(dict.projects?.next || "next").toLowerCase()} →
              </button>
            ) : (
              <span className="text-ink/30 cursor-not-allowed">
                {(dict.projects?.next || "next").toLowerCase()} →
